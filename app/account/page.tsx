          ...
          <CardBody>
            <dl className="space-y-5">
              ...
              <Field label="Reports run">
                <span className="text-ink tabular-nums">{jobs.length}</span>
              </Field>

              {oldest && (
                <Field label="First report">
                  <time className="text-ink tabular-nums" dateTime={oldest.createdAt}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-6">
      <dt className="text-ink-subtle w-32 shrink-0 text-sm font-medium">{label}</dt>
